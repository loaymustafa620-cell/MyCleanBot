...
def handle_withdraw_amount(call):
    ...
    msg = bot.send_message(call.message.chat.id,
        t(uid, "withdraw_address",
          amount=amount, gross=gross, net=net,
          fee=DOLLAR_FEE, address=wallet),
        parse_mode="Markdown")
    bot.register_next_step_handler(msg, process_withdraw_proof, amount, net, fname)

def process_withdraw_proof(message, amount, net, fname):
    uid = message.from_user.id
    # لازم يبعت صورة
    if message.content_type != 'photo':
        msg = bot.send_message(message.chat.id,
            "❌ يرجى إرسال **صورة** إثبات السحب فقط.",
            parse_mode="Markdown")
        bot.register_next_step_handler(msg, process_withdraw_proof, amount, net, fname)
        return
    ...
    # أبلّغ المستخدم
    bot.send_message(message.chat.id,
        t(uid, "withdraw_proof_received"),
        reply_markup=get_main_keyboard(uid), parse_mode="Markdown")
    # أبعت للأدمن
    try:
        bot.send_message(DEVELOPER_ID,
            t(uid, "withdraw_admin_alert",
              name=fname, uid=uid, amount=amount, net=net),
            parse_mode="Markdown")
        bot.send_photo(DEVELOPER_ID, file_id,
            caption=f"📸 إثبات سحب من {fname} (`{uid}`) — {amount}$ — {net} جنيه",
            parse_mode="Markdown")
    except Exception as e:
